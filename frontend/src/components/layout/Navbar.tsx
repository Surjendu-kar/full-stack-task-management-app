          ...
          <div className="relative" ref={menuRef}>
            ...
            <button
              ...
              className={`p-2 rounded-full transition-colors hover:text-indigo-600 text-gray-700 cursor-pointer ${
                isMenuOpen ? "text-indigo-600" : "text-gray-700"
              }`}
            >
              <FiUser className="h-6 w-6" />
            </button>
            {isMenuOpen && (
              <div className="absolute right-0 w-48 bg-white rounded-md shadow-md z-10 border border-gray-100 overflow-hidden">
                <div className="px-4 py-2 text-sm text-gray-500 border-b border-gray-200 capitalize">
                  {user}
                </div>
                <button
                  onClick={handleLogout}
                  className="w-full text-left px-4 py-2 text-sm text-gray-700 hover:text-indigo-600 hover:bg-gray-50 cursor-pointer"
                >
                  Logout
                </button>
              </div>
